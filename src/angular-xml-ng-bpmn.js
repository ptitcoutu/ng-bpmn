...
(function(ng, X2JS) {
  ...
  function responseIsXml(response) {
    ...
    if (contentType) {
      return (contentType.indexOf(XML) >= minIndex) || (response.data != null && response.data.indexOf("<?xml") >=0);
    } else {
      return (response.data != null && response.data.indexOf("<?xml") >=0);
    }
  }

  function xmlHttpInterceptorFactory($q, x2js, $log) {
